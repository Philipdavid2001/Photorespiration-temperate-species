...
length(unique(df$pairid))       # individual runs + leaf temperature
...
correct_RD <- function(data, output_path){
  ...
  for(dataIdx in 1:length(data)){
    ...
    # Calculate corrected Anet. The values for slope and intercept 
    # where calculated using values taken from the literature.  
